...
EPOCH_SIZE = 10000 * NUM_TRAIN_BATCH - int(0.95*(50000/10))
...
def read_in_all_images(address_list, shuffle = True, is_random_label = False, is_unbalance = True):
    """
    ...
    if is_unbalance is True:
        # break the balance of data distribution
        # mark the index which will be deleted
        index = 0
        del_list = []
        for i in label:
            if i == 9:
        ...
        # delete the marked examples
        data_del = np.delete(data, del_list[0:cut_num], 0)
        label_del = np.delete(label, del_list[0:cut_num], 0)
        print(np.size(label_del))
        if shuffle is True:
            print 'Shuffling'
            order = np.random.permutation(len(label_del))
            data_del = data_del[order, ...]
            label_del = label_del[order]

        data_del = data_del.astype(np.float32)
...
def read_validation_data():
    ...
    validation_array, validation_labels = read_in_all_images([vali_dir],
                                                       is_random_label=VALI_RANDOM_LABEL, is_unbalance = False)
